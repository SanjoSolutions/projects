...
function createProgressCircle({ width, height, progress, label }) {
  ...
  function draw() {
    ...
    context.beginPath()
    context.arc(center.x, center.y, radius - 0.5 * context.lineWidth, startAngle, startAngle + progress * 2 * Math.PI)
    context.stroke()
    ...
    if (label) {
      ...
      context.fillText(text, center.x - 0.5 * textWidth, center.y + 0.5 * textHeight)
    }
  }
