...
describe("replaceStringValues", () => {
  ...
  for (const xmlKey of xmlKeys) {
    it("replaces string values with its references", () => {
      ...
      expect(
        replaceStringValues(lookUp, `${xmlKey}="Login Activity"`)
      ).toEqual({ text: `${xmlKey}="@string/lgn_act"`, errors: [] });
    });
  }
  ...
  it("replaces multiple occurrences", () => {
    ...
    expect(
      replaceStringValues(
        lookUp,
        'android:text="Login Activity" android:label="Register Me Too"'
      )
    ).toEqual({
      text: `android:text="@string/lgn_act" android:label="@string/rgst_me"`,
      errors: [],
    });
  });
  ...
  it("returns an error when string value has not been found in look-up", () => {
    const lookUp = new Map([]);
    expect(
      replaceStringValues(lookUp, 'android:text="Login Activity"')
    ).toEqual({
