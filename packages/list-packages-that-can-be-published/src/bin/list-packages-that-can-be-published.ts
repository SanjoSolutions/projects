  ...
  packageFolderNames.map(async (packageFolderName) => {
    ...
    try {
      ...
      if (version) {
        ...
        canBePublished =
          !lastPublishedVersion || compare(lastPublishedVersion, version) === -1
      }
    } catch (error: any) {
...
const packagesThatCanBePublished = result.filter(
  (result) =>
    ...
    result.value.name &&
    result.value.canBePublished,
) as PromiseFulfilledResult<{ name: any; canBePublished: boolean | null }>[]
const packageNamesThatCanBePublished = packagesThatCanBePublished.map(
