...
const angle = (5 / 32) * (2 * Math.PI) // in rad (radian)
...
animate((ellapsedTime) => {
  ...
  const deltaX = speed * meterToPixelRatio * passedSeconds * Math.cos(angle)
  const deltaY = speed * meterToPixelRatio * passedSeconds * Math.sin(angle)

  const windDeltaX =
    windSpeed * meterToPixelRatio * passedSeconds * Math.cos(windDirection)
  const windDeltaY =
  ...
  if (
    -drawDelta <= x &&
    x < window.innerWidth + drawDelta &&
    -drawDelta <= y &&
    y < window.innerHeight + drawDelta
  ) {
    drawProjectile({ x, y })
    drawAngle()
    drawPowerBar()
