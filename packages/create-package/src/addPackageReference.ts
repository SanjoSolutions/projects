...
export async function addPackageReference (tsconfigPath: string, referencePath: string) {
  const json = JSON.parse(await readFile(tsconfigPath))
  json.references.push({ path: '.' + path.sep + path.relative(path.dirname(tsconfigPath), referencePath) })
