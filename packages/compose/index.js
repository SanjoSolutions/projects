...
async function composePage(userFunctions, pagePath, pageSourcePath) {
    ...
    const blockInsertPointRegExp = new RegExp(`<!-- BLOCK: (.+?) -->${newLineExpression}?`, 'g')
    let composedContent = content
    let result

    const layoutRegExp = new RegExp(`<!-- LAYOUT: (.+?) -->${newLineExpression}?`, 'g')
    let lastResult
    ...
    if (lastResult) {
        ...
        const contentInsertPointRegExp = new RegExp(`<!-- CONTENT -->${newLineExpression}?`)
        composedContent = composedContent.substring(0, lastResult.index) +
            layoutContent.replace(contentInsertPointRegExp, composedContent.substring(lastResult.index))
    }
    ...
    const boundUserFunction = Object.fromEntries(
        Object.entries(userFunctions).map(
            ([functionName, fn]) => ([functionName, fn.bind(null, compose)])
        )
    )
    const context = {...boundUserFunction}
    vm.createContext(context)
    const codeInsertPointRegExp = new RegExp(`<!-- CODE: (.+?) -->${newLineExpression}?`, 'g')
    ...
    await fs.writeFile(pageDestinationPath, composedContent, {encoding: 'utf8'})
}
