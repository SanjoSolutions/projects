...
function mockFileSystem() {
  ...
  jest.spyOn(fs, 'access').mockImplementation(async function access(filePath: PathLike): Promise<undefined> {
  ...
  jest
    .spyOn(fs, 'readFile')
    .mockImplementation(async function readFile(filePath: PathLike | FileHandle): Promise<string> {
      const content = await mockedFileSystem.getContent(filePath as string)
      return content ?? ''
    })
  jest
    .spyOn(fs, 'writeFile')
    .mockImplementation(async function writeFile(
