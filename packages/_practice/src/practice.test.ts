...
describe('practice', () => {
  ...
  test('sorting and chaining', () => {
    expect(['b', 'c', 'a'].sort().map(value => '@' + value)).toEqual(['@a', '@b', '@c'])
  })
  ...
  test('reduce array of objects', () => {
    ...
    const merge = objects.reduce((merge, object) => Object.assign(merge, object), {})
  ...
  test('pipe', () => {
    expect(pipe(1, value => value * 2, value => value + 3)).toEqual(5)
  })
