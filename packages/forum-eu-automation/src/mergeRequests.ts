...
export class MergeRequests {
  ...
  async pushFeatureBranch(ticketId: string): Promise<void> {
    ...
    const localBranchName = this.findBranchNameForTicketId(
      branchNames,
      ticketId,
    )
    if (!localBranchName) {
  ...
  findBranchNameThatStartsWith(
    branchNames: string[],
    branchNameStart: string,
  ): string | null {
    this.makeSureThatHasBeenInitialized()
    const matchingBranchNames = branchNames.filter((branchName) =>
      branchName.startsWith(branchNameStart),
    )
    if (matchingBranchNames.length > 1) {
      throw new Error(
        `Multiple branch names found that start with "${branchNameStart}".`,
      )
    } else {
  ...
  async getBranches(): Promise<{ name: string }[]> {
    ...
    const branches = stdout.split("\n").map((line) => {
      ...
      if (match) {
        ...
        throw new Error(
          `line "${line}" did not match the regular expression for a branch name.`,
        )
      }
    })
    return branches
  ...
  async openMR(ticketId: string, status: Status): Promise<void> {
    ...
    const branchNames = await gitLabAPI.listRepositoryBranches(
  ...
  async getMergeRequestForTicketId(
    ticketId: string,
  ): Promise<MergeRequest | null> {
    this.makeSureThatHasBeenInitialized()
    const gitLabAPI = this.createGitLabAPI()
    const mergeRequests = await gitLabAPI.listProjectMergeRequests(
      this.#config.projectId,
    )
    const mergeRequest = this.findMergeRequestForTicketId(
      mergeRequests,
      ticketId,
    )
    return mergeRequest
  ...
  async postReadyToReviewMessage(
    ticketId: string,
    ...
    await slackLoginPage.login(
      this.#config.slackAccount.email,
      this.#config.slackAccount.password,
    )
...
export function createMergeRequestForTicketNotFoundError(
  ticketId: string,
): Error {
...
function findStartsWith(
  ...
  return array.find(
    propertyStartsWith.bind(null, propertyName, startsWithString),
  )
}
