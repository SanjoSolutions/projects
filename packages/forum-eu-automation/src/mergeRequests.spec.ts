...
describe('MergeRequests', () => {
  ...
  describe('pushFeatureBranch', () => {
    ...
    it('throws an error when the feature branch has not been found', async () => {
      spyOnExec('* develop\n  master')
      const ticketId = '1'
      await expect(mergeRequests.pushFeatureBranch(ticketId)).rejects.toEqual(new Error('Feature branch not found.'))
    })
  })
  ...
  describe('getBranchNameThatStartsWith', () => {
    it('returns the git branch name that starts with', () => {
      ...
      expect(mergeRequests.findBranchNameThatStartsWith(branchNames, '1-')).toEqual('1-feature')
    })

    it('throws an error when multiple branch names have been found that start with', () => {
      const branchNames = ['develop', 'master', '1-feature', '1-2-feature-addition']
      expect(() => mergeRequests.findBranchNameThatStartsWith(branchNames, '1-')).toThrow(
        new Error('Multiple branch names found that start with "1-".')
      )
    })
  })
  ...
  describe('getBranchNames', () => {
    it('returns the branch names', async () => {
      spyOnExec('* develop\n  master')
      expect(await mergeRequests.getBranchNames()).toEqual(['develop', 'master'])
    })
  })
  ...
  describe('getBranches', () => {
    it('returns the branches', async () => {
      spyOnExec('* develop\n  master')
      expect(await mergeRequests.getBranches()).toEqual([{ name: 'develop' }, { name: 'master' }])
    })
  })

  describe('openMR', () => {
    it('opens a MR on GitLab', async () => {
      jest.spyOn(GitLabAPI.prototype, 'listRepositoryBranches').mockResolvedValue(['1-feature'])
      jest.spyOn(GitLabAPI.prototype, 'createMergeRequest').mockResolvedValue()
      const ticketId = '1'
      const status = 'wip'
      await mergeRequests.openMR(ticketId, status)
      expect(GitLabAPI.prototype.createMergeRequest).toHaveBeenCalledWith(projectId, {
  ...
  describe('generateMRTitle', () => {
    test('status: "wip"', () => {
      ...
      expect(mergeRequests.generateMRTitle(branchName, 'wip')).toEqual(`WIP: ${branchName}`)
    })

    test('status: "ready"', () => {
      const branchName = '1-feature'
      expect(mergeRequests.generateMRTitle(branchName, 'ready')).toEqual(branchName)
    })
  })
  ...
  describe('markMRAsReady', () => {
    ...
    beforeEach(function () {
      ...
      jest.spyOn(mergeRequests, 'getMergeRequestForTicketId').mockResolvedValue(mergeRequest)
      jest.spyOn(mergeRequests, 'removeWipFromTitle').mockResolvedValue(undefined)
      jest.spyOn(mergeRequests, 'postReadyToReviewMessage').mockResolvedValue(undefined)
    })
    ...
    it('posts ready to review message', async () => {
      await mergeRequests.markMRAsReady(ticketId)
      expect(mergeRequests.postReadyToReviewMessage).toHaveBeenCalledWith(ticketId, mergeRequest)
    })

    it('throws an error when the merge request has not been found for the ticket id', async () => {
      ;(mergeRequests.getMergeRequestForTicketId as jest.Mock).mockResolvedValue(null)
  ...
  describe('getMergeRequestForTicketId', () => {
    it('returns the merge request for the ticket id from the GitLab API ', async () => {
      jest.spyOn(GitLabAPI.prototype, 'listProjectMergeRequests').mockResolvedValue([createMergeRequest()])
      const ticketId = '1'
      const mergeRequest = await mergeRequests.getMergeRequestForTicketId(ticketId)
