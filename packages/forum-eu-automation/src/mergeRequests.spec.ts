...
import { createMergeRequestForTicketNotFoundError, MergeRequests } from './mergeRequests'
...
describe('MergeRequests', () => {
  ...
  describe('pushFeatureBranch', () => {
    ...
    it('throws an error when the feature branch has not been found', async () => {
      spyOnExec('* develop\n  master')
      const ticketId = '1'
      await expect(mergeRequests.pushFeatureBranch(ticketId))
        .rejects
        .toEqual(new Error('Feature branch not found.'))
    })
  })
  ...
  describe('getBranchNameThatStartsWith', () => {
    it('returns the git branch name that starts with', () => {
      ...
      expect(mergeRequests.findBranchNameThatStartsWith(branchNames, '1-'))
        .toEqual('1-feature')
    })

    it(
      'throws an error when multiple branch names have been found that start with',
      () => {
        const branchNames = [
          'develop',
          'master',
  ...
  describe('openMR', () => {
    it('opens a MR on GitLab', async () => {
      jest.spyOn(GitLabAPI.prototype, 'listRepositoryBranches').mockResolvedValue([
        '1-feature',
      ])
  ...
  describe('generateMRTitle', () => {
    test('status: "wip"', () => {
      ...
      expect(mergeRequests.generateMRTitle(branchName, 'wip'))
        .toEqual(`WIP: ${branchName}`)
    })
  ...
  describe('markMRAsReady', () => {
    ...
    beforeEach(function () {
      ...
      jest.spyOn(mergeRequests, 'getMergeRequestForTicketId').mockResolvedValue(
        mergeRequest,
      )
    ...
    it('posts ready to review message', async () => {
      await mergeRequests.markMRAsReady(ticketId)
      expect(mergeRequests.postReadyToReviewMessage)
        .toHaveBeenCalledWith(ticketId, mergeRequest)
    })

    it(
      'throws an error when the merge request has not been found for the ticket id',
      async () => {
        ;(
          mergeRequests.getMergeRequestForTicketId as jest.Mock
  ...
  describe('getMergeRequestForTicketId', () => {
    it('returns the merge request for the ticket id from the GitLab API ', async () => {
      jest.spyOn(GitLabAPI.prototype, 'listProjectMergeRequests')
