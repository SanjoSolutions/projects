...
async function transformDifference(
  ...
  if (isAdded(difference)) {
    ...
    const fileAPath = path.join(
      difference.path1 as string,
      difference.name1 as string
    )
    const fileBPath = path.join(
      difference.path2 as string,
      difference.name2 as string
    )
    return {
      type: "distinct",
    ...
    throw new Error(
      "Unexpected difference: " + JSON.stringify(difference, null, 2)
    )
  }
}
