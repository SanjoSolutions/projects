...
export async function diffFolders(folderAPath: string, folderBPath: string): Promise<Difference[]> {
  const differences = await compareDir(folderAPath, folderBPath, {
    compareContent: true,
    noDiffSet: false,
  })
  return await Promise.all(
    differences.diffSet!.filter(isDifferent).map(transformDifference.bind(null, folderAPath, folderBPath))
  )
}
...
async function transformDifference(
  ...
  if (isAdded(difference)) {
    return {
      type: 'added',
      filePath: path.relative(folderBPath, path.join(difference.path2 as string, difference.name2 as string)),
    }
  } else if (isRemove(difference)) {
    return {
      type: 'removed',
      filePath: path.relative(folderAPath, path.join(difference.path1 as string, difference.name1 as string)),
    }
