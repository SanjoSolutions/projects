...
import {
  ...
  createExportsForExportNamedDeclaration,
  createNamedExport,
  File,
  FileToWrite,
  isClassDeclarationThatIsExportingName,
  isExportingName,
  ...
  OutputFileType,
  removeExportsWhichAreOnlyImportedByTests2,
  seemsToBeEmptyDescribeBlockForSymbol,
...
async function readFixtureFile(test: string, path: string): Promise<File> {
...
async function readExpectedOutputFile(
  test: string,
  path: string
): Promise<FileToWrite> {
...
function generateInputFixtureFilePath(test: string, path2: string): string {
  return path.resolve(__dirname, `fixtures/${test}/input/${path2}`);
}

function generateExpectedOutputFilePath(test: string, path2: string): string {
...
describe("isVariableDeclarationThatIsExportingName", () => {
  test('`export const exportedConst = null` exports "exportedConst"', () => {
    ...
    const anExport = exportNamedDeclaration(
      variableDeclaration("const", [
        variableDeclarator(identifier(exportedConstName)),
      ])
    );
    const name = exportedConstName;
    expect(isVariableDeclarationThatIsExportingName(anExport, name)).toEqual(
      true
    );
  });
});
...
describe("isFunctionDeclarationThatIsExportingName", () => {
  test('`export function exportedFunction() {}` exports "exportedFunction"', () => {
    ...
    expect(isFunctionDeclarationThatIsExportingName(anExport, name)).toEqual(
      true
    );
  });
});
...
describe("isClassDeclarationThatIsExportingName", () => {
  test('`export class ExportedClass {}` exports "ExportedClass"', () => {
    ...
    const anExport = exportNamedDeclaration(
      classDeclaration(identifier(exportedClassName), null, classBody([]))
    );
...
describe("seemsToBeEmptyDescribeBlockForSymbol", () => {
  test("", () => {
    const path = new NodePath({} as any, {} as any);
    const name = "test";
...
describe("createExportsForExportNamedDeclaration", () => {
  ...
  test("`export const export1, export2`", () => {
    ...
    const statement = exportNamedDeclaration(
      variableDeclaration("const", [
  ...
  test("`export function export1() {}`", () => {
    ...
    const export1Name = "export1";
    const statement = exportNamedDeclaration(
      functionDeclaration(identifier(export1Name), [], blockStatement([]))
    );
    const result = createExportsForExportNamedDeclaration(filePath, statement);
    expect(result).toEqual([
      createNamedExport(filePath, export1Name, statement),
        ...
        variableDeclarator(
          objectPattern([
            objectProperty(
              ...
              arrayPattern([identifier(export1Name), identifier(export2Name)]),
              false,
              true
            ),
          ]),
  ...
  describe("`export * from './test.js'`", () => {
    it("exports all named exports from 'test.js'", () => {
      ...
      const result = createExportsForExportNamedDeclaration(
        filePath,
        statement
      );
      expect(result).toEqual([
  ...
  describe("`export * as exports from './test.js'`", () => {
    ...
    it("exports all named exports from 'test.js' as object 'exports'", () => {
      ...
      const statement = exportNamedDeclaration(
        null,
        [exportNamespaceSpecifier(identifier("exports"))],
