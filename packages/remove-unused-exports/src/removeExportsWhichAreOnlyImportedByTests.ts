...
export function removeExportsWhichAreOnlyImportedByTests2(
  ...
  const exportsWhichAreOnlyImportedByTests =
    findExportsWhichAreOnlyImportedByTests(importsAndExports)
...
function findImports(parsedFile: ParsedFile): Import[] {
  ...
  const importDeclarations = retrieveBody(parsedFile).filter(
...
export function isVariableDeclarationThatIsExportingName(
  ...
  return (
    ...
    anExport.declaration.declarations.some(
      (declarator) =>
        isIdentifier(declarator.id) && declarator.id.name === name,
    )
  )
}
...
function removeExports(
  ...
  const exportsToRemoveGroupedByFile =
    groupExportsToRemoveByFile(exportsToRemove)
  ...
  const importsToRemoveGroupedByFile =
    groupImportsToRemoveByFile(importsToRemove)
      ...
      if (isExportNamedDeclaration(statement)) {
        ...
        if (isVariableDeclaration(statement.declaration)) {
          statement.declaration.declarations =
            statement.declaration.declarations.filter((declarator) => {
              if (isIdentifier(declarator.id)) {
          ...
          const exportedName = (
            (statement.declaration as any)?.id as Identifier
          ).name
