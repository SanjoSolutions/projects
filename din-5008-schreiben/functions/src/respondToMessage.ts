  ...
  try {
    await db.runTransaction(async (transaction: firestore.Transaction) => {
      ...
      if (status !== Status.Processing && status !== Status.Processed) {
        ...
        throw new Error('The email is already processed or has already been processed.')
      }
    })

    const to = message.payload.headers.find((header: Header) => header.name === 'Reply-To').value
