...
export async function processMessage(message: any) {
  ...
  const sharingRequestMessagesForDocument = messages.filter((message: any) =>
    message.payload.headers.some(
      (header: Header) => header.name === 'Subject' && header.value === 'Share request for ‘DIN-5008 Template’'
    )
  )

  await Promise.allSettled(sharingRequestMessagesForDocument.map(respondToMessage.bind(null, gmail)))
