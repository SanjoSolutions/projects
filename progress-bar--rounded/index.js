...
function createProgressBar({ borderRadius, progress }) {
  ...
  return {
    ...
    set progress(value) {
      progress = value
      const currentValue =
        parseInt(progressBarFilling.style.width, 10) /
        parseInt(progressBar.style.width, 10)
      progressBarFilling.style.transitionDuration =
        (Math.abs(value - currentValue) / 0.01) * (1000 / 60) + "ms"
      progressBarFilling.style.width = progress * 100 + "%"
    },
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  const progressBar = createProgressBar({ borderRadius: "9px", progress: 0.1 })
