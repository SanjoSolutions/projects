...
import { OreDeposisator, OresMap } from './index.js'

specification(() => {
  const map = new OresMap(3, 3)
  const oreDeposisator = new OreDeposisator(map)
  const fields = oreDeposisator.fieldsWithDistanceToPoint({ x: 1, y: 1 }, 1)
...
specification(() => {
  ...
  const oreDeposisator = new OreDeposisator(map)
  const fields = oreDeposisator.fieldsWithDistanceToPoint({ x: 0, y: 0 }, 1)
  const expectedFields = []
  const doesEqual = equals(fields, expectedFields)
  console.log(fields)
  console.log(expectedFields)
