...
function drawGameOver({canvas, context}) {
    ...
    context.fillText(
        text, window.innerWidth / 2 - 0.5 * textMeasure.width,
    ...
    const { stop } = animate(() => {
        ...
        if (jumpy.y - cameraYOffset <= 0) {
            ...
            if (jumpData) {
                ...
                const jumpPlatform = platforms.find(platform => (
                    platform.y === jumpy.y &&
                ...
                if (jumpPlatform) {
                    ...
                    const maxOffsetX = window.innerWidth / 2 - platformDrawing.width
                    const platformX = generatePlatformX()
                    platforms.push({x: platformX, y: calculateNextPlatformY(currentTopmostPlatform.y)})
                    jump(frame)
