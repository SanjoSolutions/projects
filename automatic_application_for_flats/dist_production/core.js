...
function verifyContactData(contactData) {
  ...
  const missingRequiredFields = (0, _getMissingFields.getMissingFields)(minimumRequiredFields, contactData)
...
async function process(getBrowser, flatOfferFetchers, { intervalBetweenProcessRuns, contactData, shouldStop }) {
...
async function onFlatOffer(getBrowser, contactData, flatOffer) {
  if (!haveAppliedForFlatOffer(flatOffer) && isAFlatWeCanApplyFor(contactData, flatOffer)) {
...
async function fetchFlatOffers(getBrowser, intervalBetweenProcessRuns, flatOfferFetchers, onFlatOffer, shouldStop) {
...
function totalRent(flatOffer) {
  ...
  } else if ((0, _isNumber.isNumber)(flatOffer.coldRent) && (0, _isNumber.isNumber)(flatOffer.serviceCharges)) {
