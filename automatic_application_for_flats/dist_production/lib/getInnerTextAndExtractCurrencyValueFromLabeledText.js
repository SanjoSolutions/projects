...
async function getInnerTextAndExtractCurrencyValueFromLabeledText(
  page,
  selector,
  label
) {
  const text = await (0, _getInnerTextOfChild.getInnerTextOfChild)(
    page,
    selector
  );
  const regExp = new RegExp(
    `^${(0, _escapeTextForRegExp.escapeTextForRegExp)(label)}: (.+)$`
  );
