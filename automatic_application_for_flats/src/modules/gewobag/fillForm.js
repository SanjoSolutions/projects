...
export async function fillForm({ form, page }, contactData) {
  ...
  // IMPROVEMENT: Support all options (this just chooses "Sonstige")
  await selectOption('[id*="income"]', "#cdk-overlay-2 > div > div > ul", 1);
  await type(
    '[id*="earliestMoveInDate"]',
    formatDate(contactData.earliestDateToMoveIn)
  );
  ...
  if (wbsElement) {
    if (contactData.wbs) {
      ...
      await type(
        '[id*="wbs_valid_until"]',
        formatDate(contactData.wbs.validUntil)
      );
      await selectOption(
  ...
  const wbsSpecialHousingNeedElement = await form.$(
    '[id*="_wbs_special_housing_need_"]'
  );
  ...
  await selectRadio(
    '[id*="flatLoss"]',
    contactData.threatenedByLossOfHousing ? 0 : 1
  );
  await selectRadio(
    '[id*="firstFlat"]',
    contactData.firstTimeHousehold ? 0 : 1
  );
