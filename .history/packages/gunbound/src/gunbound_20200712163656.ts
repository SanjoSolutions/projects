...
animate((ellapsedTime: number) => {
  ...
  const ellapsedFrames = ellapsedTime * aimedFps / 1000
  passedFrames += ellapsedFrames

  const deltaX = (
    projectile.mass * speed * meterToPixelRatio / (passedFrames / aimedFps)
  ) * Math.cos(angle)
  const deltaY = (
    projectile.mass * speed * meterToPixelRatio / (passedFrames / aimedFps)
  ) * Math.sin(angle)
  ...
  const windDeltaX = (
    windSpeed * meterToPixelRatio / (passedFrames / aimedFps)
  ) * Math.cos(windDirection)
  const windDeltaY = (
    windSpeed * meterToPixelRatio / (ellapsepassedFramesdFrames / aimedFps)
  ) * Math.sin(windDirection)

  const x = initialPoint.x + deltaX + windDeltaX
  const y = initialPoint.y + deltaY + windDeltaY - gravityOfEarth * meterToPixelRatio * (passedFrames / aimedFps)
