...
animate((ellapsedTime: number) => {
  ...
  const deltaX = (
    projectile.mass * speed * meterToPixelRatio / (ellapsedFrames / aimedFps)
  ) * Math.cos(angle)
  const deltaY = (
    projectile.mass * speed * meterToPixelRatio / (ellapsedFrames / aimedFps)
  ) * Math.sin(angle)
  ...
  const windDeltaX = (
    windSpeed * meterToPixelRatio / (ellapsedFrames / aimedFps)
  ...
  point.y += deltaY + windDeltaY - gravityOfEarth * meterToPixelRatio * (passedFrames / aimedFps)
