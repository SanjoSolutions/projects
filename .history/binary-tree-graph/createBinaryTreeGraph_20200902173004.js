...
function calculateScale(devicePixelRatio, zoom) {
...
function render({ canvas, context }, { binaryTreeRendering, viewportBoundingBox, binaryTreeRenderingBoundingBox, zoom }) {
  ...
  const scale = calculateScale(window.devicePixelRatio, zoom)
...
function rerenderBinaryTree(binaryTree, { viewportBoundingBox, zoom }) {
  ...
  } = renderBinaryTree(binaryTree, { boundingBox: binaryTreeRenderingBoundingBox, scale: calculateScale(window.devicePixelRatio, zoom) })
 ...
 */
function renderBinaryTree(binaryTree, { boundingBox, scale }) {
  ...
  const totalRenderingWidth = scale * (2 * paddingToEdge + lastLevelWidth)
  const totalRenderingHeight = scale * (2 * paddingToEdge + numberOfLevels * 2 * nodeRadius + (numberOfLevels - 1) * paddingBetweenNodes)
  ...
  context.fillRect(boundingBox.x / scale, boundingBox.y / scale, canvasWidth, canvasHeight)

  context.beginPath()
  context.rect(boundingBox.x / scale, boundingBox.y / scale, canvasWidth, canvasHeight)
  context.stroke()
  ...
  */
  ...
  function calculatePositions() {
    ...
    while (nodes.length >= 1) {
      ...
      for (let index = 0; index < nodes.length; index++) {
        ...
        let x
        if (numberOfNodes === 1) {
          x = totalRenderingWidth / scale / 2
        } else {
          x = totalRenderingWidth / scale / 2 - 0.5 * levelWidth + nodeRadius + index * (2 * nodeRadius + spaceBetweenNodesOnLevel)
        }
        node.position = {
          x,
          y
  ...
  function isNodeInViewport(node) {
    const x = scale * node.position.x
    const y = scale * node.position.y
    const result = (
...
export function createBinaryTreeGraph({ min, max, step, showLabels }) {
  ...
  function setViewportBoundingBox(newViewportBoundingBox, _zoom = undefined) {
    debugger
    previousBoundingBox = viewportBoundingBox
    viewportBoundingBox = newViewportBoundingBox
    const scale = calculateScale(window.devicePixelRatio, zoom)
    const hasZoomChanged = Boolean(_zoom && _zoom !== zoom)
    if (hasZoomChanged) {
      zoom = _zoom
    }
    if (
      ...
      (binaryTreeRenderingBoundingBox.x + binaryTreeRenderingBoundingBox.width < totalRenderingWidth && viewportBoundingBox.x + scale * viewportBoundingBox.width / zoom > binaryTreeRenderingBoundingBox.x + binaryTreeRenderingBoundingBox.width) ||
      (binaryTreeRenderingBoundingBox.y > 0 && viewportBoundingBox.y < binaryTreeRenderingBoundingBox.y) ||
      (binaryTreeRenderingBoundingBox.y + binaryTreeRenderingBoundingBox.height < totalRenderingHeight && viewportBoundingBox.y + scale * viewportBoundingBox.height / zoom > binaryTreeRenderingBoundingBox.y + binaryTreeRenderingBoundingBox.height)
    ) {
  ...
  setViewportBoundingBox({
    x: -0.5 * (viewportBoundingBox.width - totalRenderingWidth / calculateScale(window.devicePixelRatio, zoom)),
  ...
  window.addEventListener('mousemove', (event) => {
    if (spacePressed && primaryMouseButtonPressed) {
      ...
      if (previousPosition) {
        ...
        const scale = calculateScale(window.devicePixelRatio, zoom)
        setViewportBoundingBox({
          x: viewportBoundingBox.x - delta.x * scale,
          y: viewportBoundingBox.y - delta.y * scale,
          width: viewportBoundingBox.width,
          height: viewportBoundingBox.height
        })
      }
  ...
  function onZoom(_zoom) {
    ...
    setViewportBoundingBox(calculateViewport(viewportBoundingBox, previousZoom, zoom), _zoom)
  }
