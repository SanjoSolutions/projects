...
export function createBinaryTreeGraph({ min, max, step, showLabels }) {
  ...
  const graphicsPosition = { x: 0, y: 0 }
  const viewportWidth = window.innerWidth
  const viewportHeight = window.innerHeight
  let visibleGraphicsPartBoundingBox = {
    x: -graphicsPosition.x, // The x-coordinate of the first left top pixel that should be rendered.
    y: -graphicsPosition.y, // The y-coordinate of the first left top pixel that should be rendered.
    width: Math.min(viewportWidth, totalRenderingWidth - graphicsPosition.x), // The width of the viewport.
    height: Math.min(viewportHeight, totalRenderingHeight - graphicsPosition.y) // The height of the viewport.
  }
  function calculateVisibleGraphicsPartBoundingBox() {
    return {
      x: -graphicsPosition.x, // The x-coordinate of the first left top pixel of the graphics that should be rendered.
      y: -graphicsPosition.y, // The y-coordinate of the first left top pixel of the graphics that should be rendered.
      width: Math.min(viewportWidth, totalRenderingWidth - graphicsPosition.x), // The width of the viewport.
      height: Math.min(viewportHeight, totalRenderingHeight - graphicsPosition.y) // The height of the viewport.
    }
  }
  ...
  function onResize({ canvas, context }) {
    setViewportBoundingBox({
      x: visibleGraphicsPartBoundingBox.x,
      y: visibleGraphicsPartBoundingBox.y,
  ...
  let result = rerenderBinaryTree(
    binaryTree,
    { viewportBoundingBox: visibleGraphicsPartBoundingBox }
  )
  ...
  function setViewportBoundingBox(newViewportBoundingBox) {
    visibleGraphicsPartBoundingBox = newViewportBoundingBox
    if (
      (binaryTreeRenderingBoundingBox.x > 0 && visibleGraphicsPartBoundingBox.x < binaryTreeRenderingBoundingBox.x) ||
      (binaryTreeRenderingBoundingBox.x + binaryTreeRenderingBoundingBox.width < totalRenderingWidth && visibleGraphicsPartBoundingBox.x + window.devicePixelRatio * visibleGraphicsPartBoundingBox.width / zoom > binaryTreeRenderingBoundingBox.x + binaryTreeRenderingBoundingBox.width) ||
      (binaryTreeRenderingBoundingBox.y > 0 && visibleGraphicsPartBoundingBox.y < binaryTreeRenderingBoundingBox.y) ||
      (binaryTreeRenderingBoundingBox.y + binaryTreeRenderingBoundingBox.height < totalRenderingHeight && visibleGraphicsPartBoundingBox.y + window.devicePixelRatio * visibleGraphicsPartBoundingBox.height / zoom > binaryTreeRenderingBoundingBox.y + binaryTreeRenderingBoundingBox.height)
    ) {
    ...
    render({ canvas, context }, { binaryTreeRendering, viewportBoundingBox: visibleGraphicsPartBoundingBox, binaryTreeRenderingBoundingBox, zoom })
  }

  setViewportBoundingBox({
    x: -0.5 * (visibleGraphicsPartBoundingBox.width - totalRenderingWidth / window.devicePixelRatio),
    y: canvas.height >= totalRenderingHeight ? -0.5 * (canvas.height - totalRenderingHeight) : 0,
    width: visibleGraphicsPartBoundingBox.width,
    height: visibleGraphicsPartBoundingBox.height
  })
  ...
  window.addEventListener('mousemove', (event) => {
    if (spacePressed && primaryMouseButtonPressed) {
      ...
      if (previousPosition) {
        ...
        setViewportBoundingBox({
          x: visibleGraphicsPartBoundingBox.x - delta.x * window.devicePixelRatio / zoom,
          y: visibleGraphicsPartBoundingBox.y - delta.y * window.devicePixelRatio / zoom,
          width: visibleGraphicsPartBoundingBox.width,
          height: visibleGraphicsPartBoundingBox.height
        })
      }
  ...
  function onZoom(_zoom) {
    ...
    zoom = _zoom
    setViewportBoundingBox(calculateViewport(visibleGraphicsPartBoundingBox, previousZoom, zoom))
  }
