...
export class OresMap {
  constructor(width, height) {
    ...
    const orePlacer = new OrePlacer2(this)
    orePlacer.placeOres()
  }
}

export class OrePlacer2 {
  constructor(map) {
    this.map = map
  }
  ...
  fieldsWithDistanceToPoint({ x: sx, y: sy }, distance) {
    ...
    if (y1 >= 0 && y1 <= this.map.height - 1) {
      for (
        let x = Math.max(0, sx - distance);
        x <= Math.min(this.map.width - 1, sx + distance);
        x++
      ) {
        fields.push({ x, y: y1 })
      }
    ...
    if (x2 >= 0 && x2 <= this.map.width - 1) {
      for (
        let y = Math.max(0, sy - distance + 1);
        y <= Math.min(this.map.height - 1, sy + distance - 1);
        y++
      ) {
        fields.push({ x: x2, y })
      }
    ...
    if (y3 >= 0 && y3 <= this.map.height - 1) {
      for (
        let x = Math.min(this.map.width - 1, sx + distance);
        x >= Math.max(0, sx - distance - 1);
        x--
      ) {
        fields.push({ x, y: y3 })
      }
    ...
    if (x4 >= 0 && x4 <= this.map.width - 1) {
      for (
        let y = Math.min(this.map.height - 1, sy + distance - 1);
        y >= Math.max(0, sy - distance + 1);
        y--
      ) {
        fields.push({ x: x4, y })
      }
