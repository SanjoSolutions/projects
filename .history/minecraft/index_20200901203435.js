...
async function main() {
  ...
  function renderColorField() {
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const _color = [angle / (2 * Math.PI), normalizedRadius, color[2]]
  ...
  function onColorFieldMouseEvent(event) {
    ...
    const radius = Math.sqrt((center.x - x) ** 2 + (center.y - y) ** 2)
    const normalizedRadius = radius / maxRadius
    color = [angle / (2 * Math.PI), normalizedRadius, color[2]]
    colorPicker.style.backgroundColor = colorToString(color)
...
function mapAngleToZeroOneInterval(angle) {
...
function angleToBetween0And2PIAngle(angle) {
  return angle % (2 * Math.PI)
}

function angleToNonNegativeAngle(angle) {
