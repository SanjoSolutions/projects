...
function render(
  { canvas, context },
  { binaryTreeRendering, viewportBoundingBox, binaryTreeRenderingBoundingBox, zoom }
) {
...
function rerenderBinaryTree(binaryTree, { viewportBoundingBox }) {
  const binaryTreeRenderingBoundingBox = calculateBinaryTreeRenderingBoundingBox(viewportBoundingBox)
  const {
...
function renderBinaryTree(binaryTree, boundingBox) {
  ...
  for (let level = numberOfLevels - 2; level >= 0; level--) {
    ...
    spaceBetweenNodes[level] =
      numberOfNodes === 1 ? 0 : (levelWidth - numberOfNodes * 2 * nodeRadius) / (numberOfNodes - 1)
  }
  ...
  const totalRenderingHeight =
    devicePixelRatio *
    (2 * paddingToEdge + numberOfLevels * 2 * nodeRadius + (numberOfLevels - 1) * paddingBetweenNodes)
...
export function createBinaryTreeGraph({ min, max, step, showLabels }) {
  ...
  let result = rerenderBinaryTree(binaryTree, { viewportBoundingBox })
  ...
  function setViewportBoundingBox(newViewportBoundingBox) {
    ...
    if (
      ...
      (binaryTreeRenderingBoundingBox.x + binaryTreeRenderingBoundingBox.width < totalRenderingWidth &&
        viewportBoundingBox.x + (window.devicePixelRatio * viewportBoundingBox.width) / zoom >
      ...
      (binaryTreeRenderingBoundingBox.y + binaryTreeRenderingBoundingBox.height < totalRenderingHeight &&
        viewportBoundingBox.y + (window.devicePixelRatio * viewportBoundingBox.height) / zoom >
          binaryTreeRenderingBoundingBox.y + binaryTreeRenderingBoundingBox.height)
    ) {
